...
public static class ProjectResourceFromEntityAssembler
{
    public static ProjectResource ToResourceFromEntity(Project entity, List<User> users) =>
        new ProjectResource(entity.Id,entity.AuditDate,entity.Name, entity.Description,entity.ProjectDate, entity.ProjectTime, entity.ProjectLocation, entity.CompanyId,users,entity.ImageUrl?.Select(img => img.Url).ToList() ?? new List<string>());
}